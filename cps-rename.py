...
def renommage(dossier=".", fichiers_exclus=[]):
	...
	for fichier in fichiers_a_renommer:
		...
		try:
			# On coupe le nom de fichier entre les deux tirets, on supprime les
			# espaces en trop et on ajoute l'extension.
			...
			# Certains titres sont en minuscules, d'autres en majuscules. On
			# homogénéise tout pour que ce soit plus propre et pratique.
			nouveau_fichier = nouveau_fichier.lower()

			try:
				os.rename("./" + dossier + "/" + fichier, "./" + dossier + "/" + nouveau_fichier)

				# Il reste des tirets dans le nom, c'est peut-être le signe d'un
				# tiret dans l'espèce d'id aléatoire en fin de nom de fichier.
